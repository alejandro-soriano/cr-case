...
#Function that determines and organizes the data used for plotting
def gen_data_i_beam(V_y,V_x,t_w,t_f_u,t_f_l,h,w_u,w_l,num):
    ...
    layout = go.Layout(
        ...
        annotations = [dict(x = origin_x - w_u/2. + w_u/10.,y = origin_y + h - t_f_u/2.,xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-18,  ay=0),
                       ...
                       dict(x = origin_x + 3*t_w, y = y_cdg - 0.1*h, xref = "x", yref = "y", text = "V",  font = dict( color = "black",  size = 14 ),showarrow = True, arrowhead=1, arrowsize=1.5, arrowwidth=1, arrowcolor='black', ax=0., ay= -40, xanchor = "center" ) ],
...
def gen_data_t_beam(V_y,V_x,t_w,t_f_u,h,w_u,num):
    ...
    trace1 = go.Scatter(
        ...
        mode='lines',
        name=r'T Beam <br> V = {0:.2f} N <br> h = {1:.0f} mm <br> w1 = {2:.0f} mm <br> t1 = {4:.0f} mm <br> t3 = {6:.0f} mm <br> A = {7:.0f} mm2 <br> cog = {8:.2f} mm <br> I = {9:.0f} mm4'.format(V_y,h,w_u,w_l,t_f_u,t_f_l,t_w,A_tot,y_cdg,I_x),
        hoverinfo='text',
        hoveron='fills',
        fill='toself', 
        fillcolor='rgba(26,150,65,0.)',
    ...
    x_par_coord = [origin_x - max([w_u/2.,w_l/2.]) - x_par_flow[x]*h/5./max([q_flange_u]) for x in range(len(x_par_flow))]
    ...
    #Points through which the parabola-shaped shear flow in the web goes
    e1_coord = [origin_x - max([w_u/2.,w_l/2.]) - h/5.*0./max([q_flange_u]),origin_y]
    e2_coord = [origin_x - max([w_u/2.,w_l/2.])- h/5.*q_flange_u/max([q_flange_u]),origin_y + h - t_f_u/2.]
    vert_coord = [origin_x -max([w_u/2.,w_l/2.])- h/5.*q_web/max([q_flange_u]),origin_y + y_cdg]
    #Solve for the parabola constants
    a_coord, b_coord, c_coord = opt.fsolve(eq_parab,[1/180.,-5./18.,713./36.],args=(e1_coord,e2_coord,vert_coord))
    ...
    #Append constants at beginning and end of the list to offset shape as much as the shear flow value at the intersection with the following element
    y_flange_u = np.linspace(0.,q_flange_u,int(num/2 + 1)).tolist() + np.linspace(q_flange_u,0.,int(num/2 + 1)).tolist()[1:]
    y_flange_l = np.linspace(0.,0.,int(num/2 + 1)).tolist() + np.linspace(0.,0.,int(num/2 + 1)).tolist()[1:]
    ...
    # q_fl_pl = [x + origin_y  - h/5.*x/max([q_flange_u,q_flange_l]) for x in q_fl]
    q_w_pl = [origin_xd -max([w_u/2.,w_l/2.])- h/5.*x/max([q_flange_u]) for x in q_w]
    ...
    #Add all the plotting entities to a data list that maps the plotting
    data_temp = [trace1, trace1_sc, trace2, trace4, trace1b, trace2b, trace4b]
    ...
    layout = go.Layout(
        ...
        annotations = [dict(x = origin_x - w_u/2. + w_u/10.,y = origin_y + h - t_f_u/2.,xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-18,  ay=0),
                       ...
                       # dict(x = origin_x - w_l/2. + w_l/10.,y = origin_y + t_f_l/2.,xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-0.06*w_l,  ay=0),
                       # dict(x = origin_x - w_l/2. + w_l/4., y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-0.1*w_l, ay=0),
                       # dict(x = origin_x - w_l/2. + w_l/2. - t_w/4., y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-0.15*w_l, ay=0),
                       # dict(x = origin_x - w_l/2. +w_l - w_l/10., y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0.06*w_l, ay=0),
                       # dict(x = origin_x - w_l/2. + w_l - w_l/4., y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0.1*w_l, ay=0),
                       # dict(x = origin_x - w_l/2. + w_l/2. + t_w/4., y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0.15*w_l, ay=0),
...
def gen_data_c_beam(V_y,V_x,t_w,t_f_u,t_f_l,h,w_u,w_l,num):

    #Calculate geometrical properties of section
    A_tot = w_u*t_f_u + t_w*(h - t_f_u - t_f_l) + w_l*t_f_l #mm2
    y_cdg = (w_u*t_f_u*(h - t_f_u/2.) + t_w*(h - t_f_u - t_f_l)*h/2. + w_l*t_f_l**2/2)/A_tot #mm w.r.t. bottom line
    x_cdg = (w_u*t_f_u*w_u/2. + w_l*t_f_l*w_l/2. + t_w*(h - t_f_u - t_f_l)*t_w/2.)/A_tot #mm #w.r.t left edge of web


    I_x = 1./12.*w_u*t_f_u**3 + 1./12.*w_l*t_f_l**3 + w_u*t_f_u*(h - y_cdg - t_f_u/2.)**2 + w_l*t_f_l*(y_cdg - t_f_l/2.)**2 + 1./12.*t_w*(h - t_f_u - t_f_l)**3 +(h - t_f_u - t_f_l)*t_w*(h/2. - y_cdg)**2  #mm4
    I_y = 1./12.*t_f_u*w_u**3 + 1./12.*t_f_l*w_l**3 + w_u*t_f_u*(w_u/2. - x_cdg)**2 + w_l*t_f_l*(w_l/2. - x_cdg)**2 + 1./12.*(h-t_f_u-t_f_l)*t_w**3 + (h-t_f_u-t_f_l)*t_w*(x_cdg - t_w/2.)**2
    I_xy = 0.


    #Compute maximum shear flows on section
    q_flange_u = V_y*(t_f_u*w_u*(h - y_cdg - t_f_u/2.))/I_x #N/mm
    q_flange_l = V_y*(t_f_l*w_l*(y_cdg - t_f_l/2.))/I_x #N/mm

    x_sc = ((h - y_cdg)*q_flange_u*w_u/2. + y_cdg*q_flange_l*w_l/2.)/V_y
    ...
    #Entitiy that plots the left-most (non-idealized) cross section
    trace1 = go.Scatter(
        x=[origin_x, origin_x + w_l ,origin_x+w_l, origin_x  + t_w, origin_x + t_w,origin_x+w_u,origin_x+w_u,origin_x,origin_x],
        y=[origin_y,origin_y,origin_y+t_f_l,origin_y+t_f_l,origin_y+h-t_f_u,origin_y+h-t_f_u,origin_y+h,origin_y+h,origin_y],
        mode='lines',
        name=r'C Beam <br> V = {0:.2f} N <br> h = {1:.0f} mm <br> w1 = {2:.0f} mm <br> w2 = {3:.0f} mm <br> t1 = {4:.0f} mm <br> t2 = {5:.0f} mm <br> t3 = {6:.0f} mm <br> A = {7:.0f} mm2 <br> cog = {8:.2f} mm <br> I = {9:.0f} mm4 <br> Xsc = {10:.2f}'.format(V_y,h,w_u,w_l,t_f_u,t_f_l,t_w,A_tot,y_cdg,I_x,x_sc),
        hoverinfo='text',
        hoveron='fills',
        fill='toself', 
        fillcolor='rgba(26,150,65,0.)',
    ...
    #Determination of the shear flow fields in the upper and lower flanges of the I-section
    y_flange_u = np.linspace(q_flange_u,0.,21).tolist()
    y_flange_u = [y_flange_u[0]] + y_flange_u
    y_flange_l = np.linspace(q_flange_l,0.,21).tolist()
    y_flange_l = [y_flange_l[0]] + y_flange_l

    #Entity that plots the upper flange shear flow
    trace2 = go.Scatter(
        x= [origin_x +t_w/2.] + np.linspace(origin_x +t_w/2.,origin_x + w_u,21).tolist(),
        y= [origin_y + h] + np.linspace(origin_y + h + h/5.*q_flange_u/max([q_flange_u,q_flange_l]),origin_y + h,21).tolist(),
        mode='lines',
        name='Shear flow [N/mm]',
        hoverinfo = 'text',
        hoveron = 'points',
    ...
    #Entity that plots the lower flange shear flow
    trace3 = go.Scatter(
        x= [origin_x +t_w/2.] + np.linspace(origin_x + t_w/2.,origin_x + w_l,21).tolist(),
        y= [origin_y] + np.linspace(origin_y - h/5.*q_flange_l/max([q_flange_u,q_flange_l]),origin_y ,21).tolist(),
        mode='lines',
        name='Shear flow [N/mm]',
        hoverinfo = 'text',
        hoveron = 'points',
    ...
    x_par_coord = [origin_x - x_par_flow[x]*h/5./max([q_flange_u,q_flange_l]) for x in range(len(x_par_flow))]
    x_par_flow = x_par_flow.tolist()
    x_par_flow.append(q_flange_u)
    x_par_flow.insert(0,q_flange_l)
    ...
    # x_par_coord = x_par_coord.tolist()
    x_par_coord.append(origin_x)
    x_par_coord.insert(0,origin_x)
    ...
    #Define the x and y coordinates of the right-most (idealized) cross section. The cross section is lumped to its midplane
    x_fu = np.linspace(origin_xd ,origin_xd+w_u,num).tolist()
    x_fl = np.linspace(origin_xd ,origin_xd+w_l,num).tolist()
    x_w =  np.linspace(origin_xd, origin_xd,num+1).tolist()
    ...
    #Append constants at beginning and end of the list to offset shape as much as the shear flow value at the intersection with the following element
    y_flange_u = np.linspace(q_flange_u,0.,num).tolist()
    y_flange_l = np.linspace(q_flange_l,0.,num).tolist()
    ...
    q_fu_pl = [x + origin_y + h + h/5.*x/max([q_flange_u,q_flange_l]) for x in q_fu]
    q_fl_pl = [x + origin_y  - h/5.*x/max([q_flange_u,q_flange_l]) for x in q_fl]
    q_w_pl = [origin_xd - t_w/2. - h/5.*x/max([q_flange_u,q_flange_l]) for x in q_w]
    ...
    layout = go.Layout(
        ...
        hovermode= 'closest',
        #Add annotations with arrows indicating the shear flow direction and the shear load application which are proportional to the cross-section size
        annotations = [dict(x = origin_x + 8*w_u/10., y = origin_y + h - t_f_u/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=36, ay=0),
                       dict(x = origin_x + 4.5*w_u/10., y = origin_y + h - t_f_u/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=54, ay=0),
                       dict(x = origin_x + t_w, y = origin_y + h - t_f_u/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=66, ay=0),
                       
                       dict(x = origin_x + 0.98*w_l, y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-36, ay=0),
                       dict(x = origin_x + 0.7*w_l, y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-54, ay=0),
                       dict(x = origin_x + 0.36*w_l, y = origin_y + t_f_l/2., xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=-66, ay=0),

                       dict(x = origin_x + t_w/2., y = origin_y + t_f_l, xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0., ay= -30 ),
                       dict(x = origin_x + t_w/2., y = origin_y + 0.85*h, xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0., ay= -30 ),
                       dict(x = origin_x + t_w/2., y = origin_y + 0.2*h, xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0., ay= -36 ),
                       dict(x = origin_x + t_w/2., y = origin_y + 0.65*h, xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0., ay= -36 ),
                       dict(x = origin_x + t_w/2., y = origin_y + 0.37*h, xref = "x", yref = "y", showarrow = True, arrowhead=2, arrowsize=1, arrowwidth=1, arrowcolor='red', ax=0., ay= -54 ),

                       dict(x = origin_x -x_sc, y = y_cdg + 0.05*h, xref = "x", yref = "y", text = "V",  font = dict( color = "black",  size = 14 ),showarrow = True, arrowhead=1, arrowsize=1.5, arrowwidth=1, arrowcolor='black', ax=0., ay= -40, xanchor = "center" ) ],
        #Define axes visibility properties
        xaxis=dict(
            autorange=False,
            showgrid=False,
...
def generate_output_callback(datasource_1_value):
    ...
    if datasource_1_value == 'T':
        def output_callback(input1,input2,input3,input4,input5,input7,input9):
            # This function can display different outputs depending on
            # the values of the dynamic controls
            data, layout = gen_data_t_beam(input3,V_x,input9,input7,input4,input5,input1)
        ...
        def output_callback(input1,input2,input3,input4,input5,input6,input7,input8,input9):
            # This function can display different outputs depending on
            # the values of the dynamic controls
            data, layout = gen_data_c_beam(input3,V_x,input9,input7,input8,input4,input5,input6,input1)
